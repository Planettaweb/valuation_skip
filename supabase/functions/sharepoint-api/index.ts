...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (action === 'download') {
      const { path } = await req.json()
      if (!path) throw new Error('No path provided for download')

      const encodedPath = path.split('/').map(encodeURIComponent).join('/')
      const downloadUrl = `https://graph.microsoft.com/v1.0/drives/${driveId}/root:/${encodedPath}:/content`
      ...
      if (!downloadRes.ok) {
        ...
        throw new Error(`SharePoint Download Failed: ${err}`)
      }
